...
class Router(Profile):
        ...
        if (not chain) and (not policy):
            for i in ["INPUT", "FORWARD"]:
                ...
                self.add_rule(
                    ACCEPT().
                    chain(i).
                    option(InInterface(self._inet_iface)).
                    match(MatchState("RELATED", "ESTABLISHED"))
                )
                # TRUSTED LAN
                self.add_rule(ACCEPT().chain(i)
                              .option(InInterface(self._inet_iface).negate()))
        else:
    ...
    def set_tcpmss(self):
        ...
        self.add_rule(
            TCPMSS().
            mss(1380).
            match(MatchTCP().tcp_flags("SYN,RST", "SYN")).
            match(MatchTCPMSS(1201, 6000))
        )
        #self.add_rule(
        #    TCPMSS().
        #    clamp_mss_to_pmtu().
    ...
    def accept_input(self, match, ranges):
        ...
        for i in ranges:
            if type(i) is tuple or type(i) is list:
                ...
                self.add_rule(
                    ACCEPT_INPUT().
                    match(match(dport=i[0], dport_end=i[1]))
                )
            else:
                self.add_rule(
                    ACCEPT_INPUT().
    ...
    def accept_input_string(self, match, strseq, delim="-"):
        ...
        self.accept_input(
            match,
            map(lambda x: x.split(delim) if delim in x else x, strseq.split())
        )

    def add_forward(self, ipaddr, port, match):
        iface_ip_list = [
            (self._inet_iface, self._inet_ip),
            (self._local_iface, self._local_ip)
        ]
        for iface, ip in iface_ip_list:
            dnat = DNAT(ipaddr=ipaddr, port=port).match(
                match).option(InInterface(iface))
            if ip:
                dnat.destination(ip)
            self.add_rule(dnat)
            self.add_rule(
                ...
                chain("FORWARD").
                match(match).
                option(InInterface(iface))
            )

    def masquerade(self):
        if self._inet_ip:
            self.add_rule(
                SNAT(ipaddr=self._inet_ip).
